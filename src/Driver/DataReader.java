...
public class DataReader {
	...
	@DataProvider(name="NumberHosting", parallel=false)
	public static Object[][] datareader3() throws IOException
	{
		
		 FileInputStream file = new FileInputStream(new File("src\\Data\\NumberHosting.xlsx"));
		 //FileInputStream file = new FileInputStream(new File("src\\Data\\AutomationInputSheet_PhaseRegressionJetUI_update.xlsx"));
...
//		//Log.info(Integer.toString(datareader().length));
	       	System.out.println(datareader3().toString());
	 Object[][] finaldata=datareader3();
	 System.out.println("Number of Quote"+finaldata.length);
	 for(int i=0;i<=finaldata.length-1;i++)
	 {
		 Object[] data2= (Object[]) finaldata[i][0];
		 System.out.println("number of product in Quote:" +i+"***"+((data2.length)-1));
